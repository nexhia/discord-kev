var compliments, insults;

module.exports = function (robot) {

  robot.client.on('message', (message) => {
    var regexInsults = /Kev insult ((@|)([\w.-]*)):?/i;
    var regexCompliments = /Kev compliment ((@|)([\w.-]*)):?/i;
    ...
    if (message.content.match(regexInsults)) {
      var insultMsg = insults[Math.floor(Math.random() * insults.length)];

      message.channel.send("<@" + user.id + "> " + insultMsg);
    }

    if (message.content.match(regexCompliments)) {
      var complimentMsg = compliments[Math.floor(Math.random() * compliments.length)];

      message.channel.send("<@" + user.id + "> " + complimentMsg);
    }
  })
}
